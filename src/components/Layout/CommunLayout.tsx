      ...
      <div className={`${!navBar ? 'flex flex-col' : 'hidden md:flex'}`}>
       ...
       <div className='flex flex-col xl:flex-row  pt-25 xl:pt-15 px-50 xl:px-35 items-center justify-between content-center'>
          <div className='my-4   xl:my-0  '> <ImgSelect selectTab={selectTab} data={data} /></div>
           <section className='w-100 md:w-125 lg:w-100 xl:w-150 text-center lg:text-left px-7 '>
               ...
               <div className='flex mt-4 md:flex  pt-2.5  md:w-120 xl:flex-col xl:w-[100%] '>
       ...
       <div className='flex flex-col   md:flex-row justify-center items-center gap-5 mt-15 p-5'>
         <article className='w-full flex  flex-row md:w-40 xl:w-60  h-25 p-4 border border-gray-400 rounded-[7px] 
          gap-2.5 items-center justify-between flex-col '>
           <div><h2 className='uppercase text-[1.2em] font-semibold '>Rotation time</h2></div>
           <div><h3 className='text-[1.2em] font-bold'>{data.rotation}</h3></div>
         </article>
         <article className='w-full flex flex-row md:w-40  xl:w-60 h-25  p-4  border border-gray-400 rounded-[7px] gap-2.5 items-center justify-between flex-col'>
            <div><h2 className='uppercase text-[1.2em] font-semibold'>Revolution time</h2></div>
           <div> <h3 className='text-[1.2em] font-bold'>{data.revolution}</h3></div>
         </article>
         <article className='w-full flex flex-row md:w-40 xl:w-60  h-25  p-4  border border-gray-400 rounded-[7px] gap-2.5 items-center justify-between flex-col'>
            <h2 className='uppercase text-[1.2em] font-semibold'>Radius</h2>
             <h3 className='text-[1.2em] font-bold'>{data.radius}</h3>
         </article>
         <article className='w-full flex flex-row md:w-40 xl:w-60  h-25  p-4  border border-gray-400 rounded-[7px] gap-2.5 items-center justify-between flex-col'>
            <div> <h2 className='uppercase text-[1.2em] font-semibold'>Average temp.</h2></div>
             <div> <h3 className='text-[1.2em] font-bold'>{data.temperature}</h3></div>
         </article>
       </div>
    </main>
      </div>
